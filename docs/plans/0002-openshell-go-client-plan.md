...
- **`loadPolicy` is a stub returning nil.** The full policy-to-SDK pipeline (`policyyaml.Load` → `policyyaml.ToProto`) is already exercised via the `-f` manifest path; the `--policy` flag is wired for flag parity but the conversion from file to `*types.SandboxPolicy` is deferred. Passing `--policy` currently has no effect on the create spec.
